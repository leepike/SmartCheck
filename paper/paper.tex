...
To illustrate, consider the example in Figure~\ref{fig:initial}.\footnote{All
  ...
  variables.}  Data type \ttp{T} is a product type containing four lists of wrapped
\ttp{Int16}s and a single wrapped \ttp{Word8}.  For our purposes, it is not important
...
\emph{SmartCheck}.  SmartCheck takes a counterexample produced by some
oracle---in these experiments, we use QuickCheck---and generically minimizes and
generalizes the counterexample, as described in the preceding bullet points.
...
requiring no shrink definitions to be provided by the user.  Indeed, there is
nothing particularly special about the example given.

...
less than 64.  The property seems reasonable at first glance, until one realizes
that due to underflow, the property can be violated.  For example, consider the
value
%
\begin{code}
B [A (-32769)] [] [] []
...
Closely related to SmartCheck is the recent Haskell library Feat.  Feat
enumerates algebraic data types in Haskell and indexing them
quickly~\cite{feat}.  For example, Feat returns the $10^{1000}$ value in the
enumeration of lists of Booleans in under a second.  Feat combines some of the
advantages of SmallCheck and QuickCheck: 

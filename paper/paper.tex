...
The most recent released version of QuickCheck provides a \ttp{genericShrink}
function for algebraic data types, so the user merely has to declare
%
\begin{code}
...
%
\noindent
to obtain shrink instances.  (The implementation relies on the same generics
package that SmartCheck does; see Section~\ref{sec:preliminaries}.)
...
QC none       & 83 & 0.013   & 22 & 0.014   & 119 & 0.023\\\hline
...
and the corresponding graph in Figure~\ref{fig:graphs}. They show four
...
which each list is truncated to 10 elements, tuple shrinking (QC tuple), also
with \ttp{Int16} shrinking disabled, and finally shrinking using QuickCheck's
\ttp{genericShrink} (qc generic).  We omit SmallCheck, since as noted, it is not
even feasible for the example with default instances.\footnote{All results
...
subVals (B (B L\sub{0} L\sub{1}) L\sub{2}) =
  Node (B (B L\sub{0} L\sub{1}) L\sub{2})
...
counterexample rather than the zeroth sub-value.  The zeroth sub-value of
\ttp{cex} is \ttp{cex} itself.  No invariant of the algorithm would be violated
by beginning with the zeroth sub-value, and in particular, the algorithm would
...
shrunk in general, unless the elements of the list are opaque themselves.  Such a
definition is possible.

Opaque types are defined by providing the method for \ttp{opaque} in the
\ttp{SubTypes} type class.

\todo{reference into with QC, no int shrinking}
...
\multirow{2}{*}{Reverse}
  & QC & 2 & 0.002         & 0 & 0.002         & 2 & 0.007 \\\cline{2-8}
  & SC & 2 & \sci{4.8}{-4} & 0 & \sci{8.1}{-4} & 2 & \sci{5.8}{-4}\\\hline\hline

\multirow{2}{*}{Div0}
  & QC & 10 & 0.001 & 6 & 0.002 & 23 & 0.002 \\\cline{2-8}
  & SC & 5 & 0.001  & 0 & 0.001 & 5 & 0.002  \\\hline\hline

\multirow{2}{*}{Heap}
  & QC & 15 & 0.016  & 1 & 0.025 & 19 & 0.045 \\\cline{2-8}
  & SC & 7  & 0.007  & 2 & 0.003 & 12 & 0.015 \\\hline\hline

\multirow{2}{*}{Parser}
  & QC &  191 & 0.013 & 195 & 0.018 & 577 & 0.044 \\\cline{2-8}
  & SC &  16  & 0.241 & 75  & 0.800 & 14  & 0.521 \\\hline

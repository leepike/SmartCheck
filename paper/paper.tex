...
In industrial uses, shrinking is essential.  Hughes, in describing commercial applications of QuickCheck, has noted that ``without it [shrinking], randomly generated
...
at the 95th percentile values.  (The curves produced by these values are not
necessarily Gaussian, so we omit standard the deviation.)
...
We focus on \emph{algebraic data types}.  Algebraic data types are
...
\paragraph{Soundness}
The extrapolation algorithm is unsound in two ways.  First, it extrapolates from a set
...
%
\noindent
The universal generalization algorithm might return the formula
\noindent
%
\begin{code}
...
%
\noindent
The algorithm could be modified to so that when attempting to generalize a
...
(\ttp{pre ==> post}).  Our goal is to find additional counterexamples in the
...
uses QuickCheck to discover an initial counterexample.
...
%
\noindent
We can look at what was generalized and what was not.  First, the last element
...
Stack Overflow asking about how to shrinking large nested data

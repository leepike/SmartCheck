...
This trouble with randomly generating large counterexamples is compounded in
composite data types, for which increasingly larger sub-values are generated in
tandem.  For example, consider the following data type and property:
%
\begin{code}
...
defined as the integers in the list \ttp{(-d)..d]}.  For a product type,
  ...
  requiring on the order of $200^4$ test cases to be executed; executing $10e^6$
  tests requires just under four minutes on a Pentium i7 at
  2.7GHz.\footnote{This is an upper-bound, since Lazy SmallCheck eliminates some

...
\begin{abstract}
QuickCheck, developed by Claessen and Hughes, is a powerful library for automatic
test-case generation.  Because QuickCheck performs random testing, some of the
counterexamples discovered are very large.  QuickCheck provides an interface for
the user to write \emph{shrink} functions to attempt to reduce the size of
counterexamples discovered.  Hand-written implementations of shrink can be
difficult to write: they may be complex, inefficient, and consist of significant
boilerplate code.  We introduce \emph{SmartCheck}, an automatic efficient
counterexample reduction algorithm.  SmartCheck reduces algebraic data using
generic search heuristics to efficiently find smaller counterexamples
independent of the program or property being tested.  In addition to shrinking,
SmartCheck generalizes counterexamples by returning a \emph{formula} that
abstracts over individual counterexamples and make it easier for the programmer
to uncover multiple counterexamples at once.  SmartCheck has been implemented
for Haskell and is freely available.
\end{abstract}
...
counterexamples more understandable: the other side is generalization.  For
example, the following is a relatively small counterexample returned when
QuickChecking a property in Xmonad (with a bug inserted), a popular X11 window
manager written in Haskell~\cite{xmonad}:
%
\medskip%
\begin{sloppypar}
%\footnotesize
...
counterexample like the following were returned by the testing framework:
%
\begin{code}
...
with respect to the property, meaning that for any well-typed values that
replace \ttp{x0}, \ttp{x1}, \ttp{x2}, and \ttp{x3}, the counterexample still
holds.

\paragraph{SmartCheck}

Motivated by the problems of reducing and generalizing large counterexamples, we
have developed \emph{SmartCheck}.  SmartCheck takes a counterexample produced by
some oracle---in these experiments, we use QuickCheck---and generically
minimizes and generalizes the counterexample.
...
shrink implementations for some properties and programs.  More importantly, the
time required to shrink scales linearly with the size of the
counterexample---other common shrink implementations often scale in polynomial
time or worse.  (Shrinking time is important in real-world uses; see
Section~\ref{sec:experiments} for a real-world example.)  And the
counterexamples returned are two to five times smaller.
...
(\ttp{Right True}) for the type (\ttp{Either Int Bool}) means there exists a
counterexample regardless of the variant chosen.
...
example, a property that parsing a pretty-printed abstract syntax tree (AST)
results in the original AST may fail due to multiple bugs.  During testing, it
is useful to discover all these possibilities in one go.  In practice, the
...
believes she has isolated the error.
...
wrapped \ttp{Int16}s and a single wrapped \ttp{Word8}.  For our purposes, it is
not important what \ttp{T} models, but for concreteness, suppose it models the
values in four linked lists in an operating system, together with an 8-bit
status register.
...
implements implication from the QuickCheck library).
...
is null, so \ttp{t} will not be shrunk.  More troublesome is that with
...
take over 15 seconds and result in a final counterexample containing nearly 60
\ttp{N} constructors.\footnote{All results reported in the paper are with a
  GHC-compiled program, using \ttp{-O2}, on a 4-core 2.7GHz Intel~i7 processor.}
We would like to do better, particularly since the data type and property are so
simple.
...
To motivate the benefits of SmartCheck reduction algorithm, consider the graphs
in Figure~\ref{fig:graphs} and the Table~\ref{table:results}, giving the results
...
feasible for the example).  We graph counterexample size and execution time.
Out of 2000 runs, we show the number of runs resulting in a shrunk
counterexample of a given size (by counting \ttp{N} constructors), and we show
the number of runs executing within a given time limit (in seconds).  Note that
we provide two execution-time graphs, since the scale of time scales differ
dramatically.  In the table, we show the mean, median, median, and the results
at the 95th percentile values.  (The distributions produced by these values are
not necessarily Gaussian, so we omit the standard deviation.)
...
approaches, with a very small tail.
...
by QuickCheck; thus the time required by SmartCheck to shrink a counterexample
is on average just two hundredths of a second (0.10 - 0.08).  As can be seen in
the graphs, SmartCheck's execution time scales linearly (like QuickCheck,
without shrinking).  Truncation and tuple shrinking produce very long tails; the
longest execution time recorded for truncation shrinking and tuple shrinking are
88 seconds and 110 seconds, respectively.  SmartCheck is largely insensitive to
the size of the original counterexample provided.

The results for SmartCheck are fairly generalizable to other types and
properties since SmartCheck's reduction algorithm has no dependencies on the
specifics of either; however, user-defined shrink methods may vary significantly
based on the property and counterexample type.


Finally, note that after shrinking the counterexample, SmartCheck will also
generalize it.  Note that for any \ttp{Word8} value in the first argument of
\ttp{T}, the counterexample still holds.  Thus, we can be more informative and
return a quantified formula rather than a single counterexample:
%
\begin{code}
...
\item \ttp{constrs} returns the list of all possible constructors from the
  value's type,
\item \ttp{opaque} is true when the type of the value is an ``uninteresting
  type'', which generally means it is user-defined and has positive arity.
  We discuss the purpose of opaque types in Section~\ref{sec:base}.
...
In this section, we describe how to efficiently and generically shrink algebraic data
values.  Before we begin, recall the basic idea behind the \ttp{shrink} method
of the \ttp{Arbitrary} class: generate a list of values, each of which is
smaller than the current counterexample.  Each of the new values generated
typically bears no relationship to the original counterexample other than being
smaller.
...
that are strictly smaller according to the given measure to check in the graph.
...
from variant \ttp{V}.  Recall that the algorithm begins at the \emph{first} sub-value of
the counterexample rather than the 0th sub-value.  The 0th sub-value of
\ttp{cex} is \ttp{cex} itself.  No invariant of the algorithm would be violated
by beginning with the 0th sub-value, and in particular, the algorithm would
still terminate.  However, we begin with a strict sub-value because of the
hypothesis.  For example, consider a property about unintended variable capture
over a language's parse tree represented by a sum type with constructors for
module imports, function definitions, and global-variable assignments,
respectively.  A function definition counterexample can only be reduced to
smaller function definition counterexamples, the only construct in which
variable capture is possible.
...
By default, this includes types without constructors (e.g., \ttp{Int}, \ttp{Word},
...
Conversely, if the user wishes for types that are treated as opaque by default
by SmartCheck to be shrunk, the easiest approach is to define shrink instances
that operate on the opaque types while leaving the rest of the value constant.
These types are shrunk relatively efficiently by conventional QuickCheck's
...
check needs only to compare indexes, so it is efficient.  The \ttp{subTrees}
guard prevents the algorithm from trying to generalize sub-values that are
...
Second, note that each index is generalized with respect to the concrete
sub-values that exist in the counterexample.  So in some cases, a formula may be
returned that is overly general.  For example, consider the counterexample in
which both fields of \ttp{Add} contain values causing the failure:
%
\begin{code}
Add (Div (C 1) (C (-2)) (C 2))
    (Div (C 0) (C (-1)) (C 1))
...
minimizing the counterexample first, as pointed out to this author by John
Hughes, so in practice with SmartCheck, this form of unsoundness does not arise.
However, if it does, for universally quantified formulas, the reader should read
it as shorthand for quantifying each variable independently and taking the
conjunction of formulas.  For example, instead of
%
\begin{code}
forall values x0 x1 . Add x0 x1
\end{code}
...
abstract it.  For example, suppose that \ttp{prop\_div} had no precondition
\ttp{divSubTerms} testing for the occurrence of a constant (\ttp{C 0}) in the
divisor.  Then the following would all counterexamples:
%
\begin{code}
Div (Add (C 3) (C 2)) (C 0)
Div (C 7) (Add (C (-5)) (C 5))
...
constructors already seen.  Thus, (\ttp{go v}) returns (\ttp{Just True}) if
(\ttp{replace cex idx v}) passes this precondition (and any other preconditions
of \ttp{prop}), but fails the property.
...
fail the post-condition, and the enclosing ring represents the generalization of
...
\end{code}
%
The first equation holds because we ignore opaque types.  The second equation
...
have the same types.  (We only require \ttp{Read} because standard QuickCheck
does not return a counterexample; we read the shown values.)
...
Our experience is that often a program operates over one large structure
...
QuickCheck's \ttp{property} function.  SmartCheck's property implication uses an
arrow \ttp{-->} corresponding to QuickCheck's \ttp{==>}.  The reason that we
cannot just use QuickCheck properties directly is that we must do some
``pre-processing'' to extract the arguments from a counterexample.
...
Here we describe three experiments using SmartCheck, including an XMonad
...
%
\noindent
Now, suppose the programmer makes a simple typo (or perhaps has a more serious
misunderstanding) and instead writes
%
\begin{code}
removeFromWorkspace ws =
  ws \{ stack = stack ws >>= filter (==w) \}
...
\ttp{arbitrary} instances are already defined, but we had to ensure all relevant
types derived the \ttp{Read}, \ttp{Show}, \ttp{Typeable}, and \ttp{Generic}
type classes as well, together with \ttp{SubTypes}.  The work does not require
understanding the program, but it does require hunting for definitions and
multiple rounds of recompilation.
...
A few other researchers have also investigated counterexample shrinking.  Zeller
...
their work shrinking HTML inputs to crash Mozilla and shrinking C programs to
...
Daikon infers assertions for C, C++, Java, and Perl by observing relationships
between variables in executions of a program~\cite{daikon}.  SmartCheck does not
attempt to infer properties like these tools do, so it is in one sense less
general.  However, SmartCheck's three kinds counterexample generalization are novel.
...
%
\noindent
Discovering and exploiting sharing is future work.
...
I thank Joe Hurd, John Launchbury, Simon Winwood, and especially John Hughes for
reading an earlier draft of this paper.  Their deep insights and bug catches
dramatically improved both the form and content.

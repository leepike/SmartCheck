...
\title{SmartCheck: Automatic and Efficient Counterexample Reduction and Generalization\\$\;$\\DRAFT}
...
program being evaluated.  Bad definitions can be so inefficient that they are
unusable.

In addition, shrinking is one side of the coin when it comes to making counterexamples more
understandable.  The other side is generalization.  For example, the following
is a relatively small counterexample returned when QuickChecking an (incorrect)
implementation in Xmonad, a popular X11 window manager written in
Haskell~\cite{xmonad}:
%
\medskip%
\begin{sloppypar}
...
\medskip%
\noindent
Functional programmers should be familiar with having to debug a ``wall of
text'' when showing data structures.  What if instead a counterexample like the
following were returned by the testing framework:
%
\begin{code}
...
with respect to the counterexample, meaning that for any well-typed values that
...
time or worse.  And the counterexamples returned are two to five times smaller.
...
fails long before it reaches a depth required to discover a counterexample.  (A
related library named Feat combines some aspects of SmallCheck and
QuickCheck~\cite{feat}; we discuss it in Section~\ref{sec:related}).
...
Here we provide some illustrative comparisons between SmartCheck's shrinking
against other shrinking approaches.  As a representative example, we use the
example presented in Section~\ref{sec:intro}.  The results for SmartCheck are
fairly generalizable to other types and properties since SmartCheck's reduction
algorithm has no dependencies on the specifics of either; however, user-defined
shrink methods may vary significantly based on the property and counterexample
type.
...
To motivate the benefits of SmartCheck, consider the graphs in
...
not necessarily Gaussian, so we omit standard the deviation.)
...
is on average just 0.2 seconds (0.10 - 0.08).  As can be seen in the graphs,
SmartCheck's execution time scales linearly (like QuickCheck, without
shrinking).  Although it is difficult to tell from the graphs, truncation and
tuple shrinking produce very long tails, with some counterexamples taking over a
minute to shrink.  SmartCheck is largely insensitive to the size of the original
...
We focus on algebraic data types represented as ``sums of products''.  We
sometimes refer to the elements of a sum type as a \emph{variant} that is
\emph{tagged} by a constructor.  For example, the type
%
\begin{code}
...
\item \ttp{constrs} returns the list of all possible constructors from the
  value's type,
\item \ttp{baseType} is true when the type of the value is an ``uninteresting
...
to check in the graph.  However, it does not exploit that the original
counterexample \ttp{cex} is already in a neighborhood of other likely
counterexamples.  So rather than choose random vertexes, it may be more
efficient to test other values that are closely related to \ttp{cex}.  By
restricting the search to \ttp{cex}'s neighborhood, a smaller counterexample
might be missed, and only a local minimum is returned.  However, with a large
set of values, finding a global minimum by random sampling vertexes in the graph
is possible but too inefficient, as we saw from the example in the introduction.
...
\ttp{cex}, and the property \ttp{prop}.  At the beginning of execution, we
assume that a counterexample has been obtained somehow; our implementation
relies on QuickCheck to generate an initial counterexample and to generate
arbitrary well-typed random values (by the \ttp{arbitrary} method).  The
reduction begins at the first proper sub-value of \ttp{cex}; call it \ttp{v}.
When the index \ttp{idx} becomes out-of-bounds and returns \ttp{Nothing}, the
algorithm terminates.  Otherwise, a list of new random values are generated.
%
\begin{code}
...
prevents the algorithm from analyzing, for example, strings.  A list of
non-base-types, however, is shrunk by the algorithm.
...
a set of counterexamples to a universal claim, similar to QuickSpec~\cite{qs}.
By tuning the parameters, the risk of an unsound generalization is reduced by
requiring more or larger values to fail the property.
...
%
\noindent
The algorithm could be modified so that when attempting to generalize a
sub-value at an index, all previously-generalized indexes are re-generalized
simultaneously, by taking the cross-product of random values for the current
index and for the previously-generalized indexes.  While this reduces the
potential for over-generalizing a counterexample, it is exponential in the
number of indexes generalized.  For universally quantified formulas, the reader
should read it as shorthand for quantifying each variable independently and
taking the conjunction of formulas.  For example, instead of
%
\begin{code}
forall values x0 x1 . Add x0 x1
\end{code}
...
example, the following are all counterexamples:
%
\begin{code}
Div (C 7) (Add (C (-5)) (C 5))
Div (Add (C 3) (C 2)) (C 0)
...
Figure~\ref{fig:cex-gen} illustrates a property of the form (\ttp{pre ==>
  post}).  Points are specific counterexamples 

 the situation in which we have some values
and a property in the case that some values fail the property of the form
.  Our goal is to find additional counterexamples in the
shaded region, in which \ttp{pre} is satisfied, \ttp{post} is not satisfied, and
...
does not return a counterexample; we read the shown value.)
...
generated, and the algorithm depends on \ttp{arbitrary} to generate new
well-typed values.
...
be analyzed simultaneously by SmartCheck, it is not obvious how to clearly
represent multiple large counterexamples to the user---the implementation
currently prints to standard out.  (In future work, a web-based interface could
help.)
...
above to control SmartCheck's behavior.  The standard arguments are contained in
the value \ttp{scStdArgs}.
...
This is a relatively small counterexample, but even the smallest \ttp{StackSet}
values are somewhat large due to the number of fields within it.  But let us see
if we can do better.

SmartCheck returns a smaller counterexample, and then generalizes it:
%
\begin{code}
forall values x0 x1 x2 x3:
  StackSet
...
of \ttp{StackSet} is a map, and we treated it as a base type, so it is not
...
I thank Joe Hurd, John Launchbury, and John Hughes for insightful comments on
earlier drafts of this paper.  In particular, John Hughes discovered errors and
omissions that we have fixed.

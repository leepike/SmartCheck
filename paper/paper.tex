...
insight, but it reduces the sheer size of the counterexample, and it can do so significantly.
...
We call the kind of formula just shown \emph{universal sub-value generalization}
and is implemented in SmartCheck.  The algorithm is shown in
Figure~\ref{fig:universal}.  The algorithm is similar to the reduction algorithm
in Figure~\ref{fig:reduction} (and in the implementation, the algorithms are
generalized and combined).  The function \ttp{extrapolate} takes a
counterexample \ttp{cex} and a property.  It returns a list of indexes to be
universally quantified.  This algorithm also performs a breadth-first search
over the sub-values of the counterexample.  For each sub-value, the algorithm
generates new sub-values, replaces them in \ttp{cex}, and tests them.  If every
new counterexample non-trivially fails (i.e., does not just fail the
precondition) the property \ttp{prop}, then we extrapolate, claiming that for
\emph{any} new value replacing that sub-value in \ttp{cex}, the property will
fail.

The function \ttp{extrapVals} generates the new sub-values, much like
\ttp{newVal} in the reduction algorithm.  Here, \ttp{maxExtrap} well-typed
values are generated, and \ttp{unsizedArbitrary} is the analogue of
\ttp{sizedArbitrary}, but does not limit the size of values generated.

In the definition of \ttp{allFail},
%
\begin{code}
...
%
\noindent
is the roughly the dual of \ttp{pass} in the reduction algorithm: it returns
\ttp{Just True} if a value satisfies the property and passes the property's
precondition \ttp{pre}, \ttp{Just False} if the value fails the property, and
\ttp{Nothing} if the value fails \ttp{pre}.
